...
export function ProductCard({ perfume }: Props) {
  return (
    <Link to={`/perfume/${perfume.slug}`} className="group block">
      ...
      <div className="flex justify-between items-start gap-4">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground uppercase tracking-widest mt-1">{perfume.familia}</p>
        </div>
        <span className="text-sm font-mono whitespace-nowrap">{formatPreco(perfume.preco)}</span>
      </div>
    </Link>
  );
}
