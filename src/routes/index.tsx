...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Maryar — A arquitetura do invisível" },
      {
        name: "description",
        content:
          "Curadoria de fragrâncias de nicho, skincare e maquiagem. Descubra perfumes por família olfativa, estação e ocasião.",
      },
      { property: "og:title", content: "Maryar — A arquitetura do invisível" },
      {
        property: "og:description",
        content: "Curadoria de fragrâncias de nicho, skincare e maquiagem.",
      },
    ...
    <>
      ...
      <section className="relative h-[88vh] min-h-[640px] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-4xl px-6 animate-reveal">
          ...
          <p className="text-sm md:text-base tracking-wide max-w-md mx-auto leading-relaxed text-muted-foreground mb-10">
            Uma curadoria de fragrâncias que operam no silêncio entre a pele e o
            ar. Minimalismo perolado em cada nota.
          </p>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24 md:py-32">
        <div className="flex items-end justify-between mb-16">
          ...
          <Link
            to="/catalogo"
            className="text-[10px] uppercase tracking-[0.2em] border-b border-foreground/30 pb-1 hover:border-foreground transition-colors"
          >
            Ver tudo
          </Link>
        </div>
      ...
      <section id="estacoes" className="max-w-7xl mx-auto px-6 py-24 md:py-32">
        <div className="mb-16 max-w-2xl">
          <p className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-4">
            Curadoria por estação
          </p>
          <h2 className="font-serif text-4xl md:text-5xl italic mb-4">
            O ano em quatro respirações.
          </h2>
          <p className="text-sm text-muted-foreground leading-relaxed">
            Cada estação pede uma matéria-prima diferente. Encontre fragrâncias
            calibradas para o clima e a luz.
          </p>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-foreground/5">
          {estacoesCards.map((e) => (
            <Link
              key={e.nome}
              to="/catalogo"
              search={{ estacao: e.nome } as never}
      ...
      <section className="bg-paper py-24 md:py-32">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-16 max-w-2xl">
            <p className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-4">
              Curadoria por ocasião
            </p>
            <h2 className="font-serif text-4xl md:text-5xl italic">
              Um perfume para cada momento.
            </h2>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-foreground/5">
            {ocasioesCards.map((o) => (
              <Link
                key={o.nome}
                to="/catalogo"
                search={{ ocasiao: o.nome } as never}
      ...
      <section className="max-w-7xl mx-auto px-6 py-24 md:py-32 border-t border-foreground/5">
        <p className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-12 text-center">
          Marcas em destaque
        </p>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8 items-center justify-items-center text-muted-foreground/80">
          {marcas.map((m) => (
            <span
              key={m}
              className="font-serif italic text-xl text-center hover:text-foreground transition-colors"
            >
              {m}
            </span>
          ))}
        ...
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-12">
          <div className="md:col-span-1">
            ...
            <h2 className="font-serif text-3xl md:text-4xl italic">
              Sobre matéria, memória e rastro.
            </h2>
          </div>
          <div className="md:col-span-2 grid sm:grid-cols-2 gap-8">
            {[
              {
                cat: "Perfumaria",
                t: "Por que a íris é a flor mais cara da perfumaria.",
              },
              {
                cat: "Skincare",
                t: "Rituais lentos: a pele em camadas finas.",
              },
            ].map((a) => (
              <article
                key={a.t}
                className="border-t border-foreground/10 pt-6 cursor-pointer group"
              >
                <p className="text-[10px] uppercase tracking-widest text-muted-foreground mb-3">
                  {a.cat}
                </p>
                <h3 className="font-serif text-xl leading-snug group-hover:italic transition-all">
                  {a.t}
                </h3>
                <span className="inline-block mt-6 text-[10px] uppercase tracking-[0.2em] border-b border-foreground/20 pb-1">
                  Ler artigo
                </span>
              </article>
            ))}
